...
class NotificationService : Service() {
    ...
    // ✅ Tambahan dari instruksi aslab (langkah 5)
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        val returnValue = super.onStartCommand(intent, flags, startId)

        // Ambil channel ID dari intent yang dikirim MainActivity
        val Id = intent?.getStringExtra(EXTRA_ID)
            ?: throw IllegalStateException("Channel ID must be provided")

        // Jalankan proses di background thread (serviceHandler)
        serviceHandler.post {
    ...
    // Fungsi hitung mundur notifikasi dari 10 ke 0
    ...
    // Fungsi untuk kirim sinyal ke MainActivity lewat LiveData setelah selesai
